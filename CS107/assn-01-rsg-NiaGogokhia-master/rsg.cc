...
//generate text
void generateText(vector<string> &generatedText, map<string, Definition> &grammar, Definition &currDef){
  Production currProduct = currDef.getRandomProduction();

  for(vector<string>::iterator itr = currProduct.begin(); itr!=currProduct.end(); itr++){
    string currPhrase = *itr;
    if(isDef(currPhrase))generateText(generatedText, grammar, grammar[currPhrase]);
    else generatedText.push_back(currPhrase+" ");
  }
}
...
int main(int argc, char *argv[])
{
  if (argc == 1) {
    ...
    return 1; // non-zero return value means something bad happened su
  }
  ...
  for(int i=1; i<=3; i++){
    ...
    generateText(generatedText, grammar, grammar["<start>"]);
    cout << "Version # " << i << ": "<<endl;
    for(int k = 0; k < generatedText.size(); k++)cout<<generatedText[k]+" ";
    printf("\n");
  }
  return 0;
}
