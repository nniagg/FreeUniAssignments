...
(define (intersections-helper c circles) 
	(if (null? circles) 
		'()
  	(append (intersect c (car circles)) (intersections-helper c (cdr circles)))
  )
)
...
(define (intersection-points circles)
   (if (null? circles) 
   	'()
    (append (intersections-helper (car circles) (cdr circles)) (intersection-points (cdr circles)))
  )
)

;; distance product

(define (distance-product c points)
	(apply * 
		(map 
			(lambda (p) (if (equal? c p) 1 (dist p c))) 
			points
		)
	)
)
...
(define (rate-points points)
  (map 
  	(lambda (p) (list (distance-product p (remove p points)) p)) 
  	points
  )
)
...
(define (clumped-points points)
	(map 
		(lambda (p) (car (cdr p))) 
		(prefix-of-list
			(sort-points(rate-points points))
...
(define (get-y pair) (car (cdr pair)))
...
(define (best-estimate points)
	(average-point (clumped-points 
...
(define (where-am-i distances stars)
	(quicksort (map 
		(lambda (guess)(best-estimate guess))  
